...
switch ($_SESSION['registration']['$is_this_your_Shipping_address']) {
  case "on":
    if(empty($fname) || empty($lname) ||empty($zip) ||empty($password) ||empty($phone_number) ||empty($city) || empty($email) || empty($zip) ||  empty($state) || empty($streetaddress1) || empty($streetaddress2)){
